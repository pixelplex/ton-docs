...
For this guide we provide examples on these languages - `python3`, `TypeScript` and the corresponding SDKs: `@ton/ton`, `pytoniq`. First, visit installation pages and install runtime environment and package managers for your OS:
...
<TabItem value="pytoniq" label="Python3">
```bash
python3 --version
pip3 --version
```
...
<TabItem value="pytoniq" label="pytoniq">
```bash
python3 -m venv .venv
source .venv/bin/activate
python3 -m pip install pytoniq
```
</TabItem>
</Tabs>
...
async def main():
    address = Address('0QD-SuoCHsCL2pIZfE8IAKsjc0aDpDUQAoo-ALHl2mje04A-') # replace with any address
    config = {
        'timeout': 10,
        'trust_level': 1
    }

    async with LiteClient.from_testnet_config(**config) as client:
        state = await client.get_account_state(address)

        print(f"Balance: {state.balance} TON")
...
async def main():
    client = LiteBalancer.from_testnet_config(trust_level=1, timeout=300)

    await client.start_up()

    # Building optional get method parameters list
    addr = Address('0QD-SuoCHsCL2pIZfE8IAKsjc0aDpDUQAoo-ALHl2mje04A-')
    request_stack = [addr.to_cell().begin_parse()]

    result = await client.run_get_method_remote(
        address="kQD0GKBM8ZbryVk2aESmzfU6b9b_8era_IkvBSELujFZPsyy", # address to call get method on
        method='get_wallet_address', # method name
        stack=request_stack # optional params list
    )

    print(result[0].load_address())
...
async def main():
    ...
    # calling get method from wrapper
